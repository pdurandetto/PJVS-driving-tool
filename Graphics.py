...
from numpy import diff, arange, sign
...
from System.Windows.Forms import Application, Form, Button, Label,TextBox, StatusBar, MessageBox, Panel, RadioButton, DataVisualization, CheckBox

Graph = DataVisualization.Charting

sim= Settings.SIMULATION #global variable set in Main.py
n_ch = Settings.N_CHANNELS
n_subs = n_ch-1 #number of subsections is equal to the number of channels minus one, 14
...
class PJVS_GUI(Form):
        def __init__(self):
            ...
            self.plotVt.Location = Point(350, 10)
            self.plotVt.Size = Size (570, 430)
            ...
            #radiobutton for selecting the waveform type
            self.radioLabel_wave_type = Label()
            self.radioLabel_wave_type.Text = "Waveform type"
            self.radioLabel_wave_type.Font = Font("Serif", 12.)
            self.radioLabel_wave_type.Location = Point(360, 460)
            self.radioLabel_wave_type.AutoSize = True

            self.radio_sin = RadioButton()
            self.radio_sin.Text = "Sinewave"
            self.radio_sin.Font = Font("Serif", 12.)
            self.radio_sin.Location = Point(370, 490)
            ...
            self.radio_sqr.Location = Point(370, 530)
            ...
            self.radio_trng.Location = Point(370, 570)
            self.radio_trng.Size = Size (120, 50)
            self.radio_trng.CheckedChanged += self.checked_waveform

            self.Controls.Add(self.radioLabel_wave_type)
            self.Controls.Add(self.radio_sin)
            self.Controls.Add(self.radio_sqr)
            self.Controls.Add(self.radio_trng)
            ...
            self.btn_LOAD.Location = Point(590, 500)
            ...
            self.btn_RUN.Location = Point(760, 500)
            ...
            #Button on the first form for opening the second form, where IV characteristics can be performed
            self.btn_IV = Button()
            self.btn_IV.Text = "Check IVs"
            self.btn_IV.Font = Font("Serif", 12.)
            self.btn_IV.BackColor = Color.White
            self.btn_IV.Location = Point(200, 40)
            self.btn_IV.Size = Size (100, 50)
            if sim:        
...
########################################################################### end of constructor
        ...
        def Set_in_volt_Format(self, sender, args):
            number0 = '{0:.6f}'.format(self.Call_QuantVolt(self,self)[0])
            number1 = '{0:.6f}'.format(self.Call_QuantVolt(self,self)[1])
            self.in_amp_signal.Text = str(number0)
            self.in_offset.Text = str(number1)
        ...
        def Set_in_n_steps_Format(self, sender, args):
            number = '{0:d}'.format(round(float(self.in_n_steps.Text)))
            self.in_n_steps.Text = str(number)
            self.Set_in_freq_signal_Format(self,self)


        #it quantizes amplitude and offset of the output waveform
        def Call_QuantVolt(self, sender, args):
            Vamp = float(self.in_amp_signal.Text)
            Voffset = float(self.in_offset.Text)            
            Vamp_q= Math_Functions.CalcQuantumVolt(Vamp, self.single_jj_volt, 50, float(self.in_current.Text)*0.001, n_subs)[1] #current in A, element 1 of the output of CalcQuantumVolt
            Voffset_q= Math_Functions.CalcQuantumVolt(Voffset, self.single_jj_volt, 50, float(self.in_current.Text)*0.001, n_subs)[1]
            return([Vamp_q,Voffset_q])
        ...
        def Call_Eval_Parameters(self, sender, args):
            ...
            Param_array = Math_Functions.Eval_Parameters(freqRF, n_subs)
        ...
        def Call_Load(self, sender, args):
                ...
                V_1 = self.single_jj_volt
                R_out = float(50) #output impedance 50 Ohm
                I_bias = (float(self.in_current.Text))*10**(-3) # *10**(-3) ----> from mA to A
                
                                
                if self.waveform == "SIN":
                    Vout = Math_Functions.Calc_Sine(amp, freq, phase, offset, n_steps, V_1, R_out, I_bias, n_subs)
                    self.Vmatrix = Vout[0]
                    
                if self.waveform == "SQR":
                    Vout = Math_Functions.Calc_Square(amp, freq, phase, offset, n_steps, V_1, R_out, I_bias, n_subs)
                    self.Vmatrix = Vout[0]
                    
                if self.waveform == "TRN":
                    Vout = Math_Functions.Calc_Triang(amp, freq, phase, offset, n_steps, V_1, R_out, I_bias, n_subs)
                    self.Vmatrix = Vout[0]
                ...
                if not sim:
                    ...
                    if result == "ERR_IMP":
                        MessageBox.Show("WARNING: Output impedance not correctly set!")
                        return(-1)
                    else:
...
#************************************************* IV-CHARACTERISTIC FORM **********************************************            
        ...
        def OpenFormIV(self, sender, args):
            ...
            self.form_IV.npointsIV.Location = Point(10, 210)
            ...
            self.form_IV.in_npointsIV.Text = "30"
            self.form_IV.in_npointsIV.Location = Point(185, 210)
            ...
            self.form_IV.list_checkbox_all.Location = Point(270,80)
            ...
            for i in range(len(checkbox_subjj)):
                ...
                self.form_IV.list_checkbox_subjj[i].Location = Point(270,int(105+i*25))
            ...
            self.form_IV.panel_subjj.Location = Point(260,70) 
        ...
        def Call_InitVoltmeter(self, sender, args):
            self.voltmeter = Devices_Control.InitVoltmeter() #call InitVoltmeter from Devices_Control
            if self.voltmeter == "ERR_VOLT":
                self.form_IV.sb_IV.Text = "Result: Voltmeter not found!"
                return(-1)
            else:
                self.form_IV.sb_IV.Text = "Result: Voltmeter Keithley 2182A correctly initialized"
        ...
        def RunIV(self, sender, args):
                ...
                pars = Math_Functions.Eval_Parameters(float(self.in_freqRF.Text), n_subs)
                ...
                n_pts = int(int(self.form_IV.in_npointsIV.Text)/2.)
                
                dV = float(Vmax/n_pts) #voltage variation            
                ...
                while self.running:
                    ...
                    if first_span:
                        ...
                        Vch = Vlist_new[i] #Vlist_new is the same of Vlist but without the points where V>Vmax and the measured current is greater than curr_max, it is created after the first span
                    ...
                    if first_span:
                        ...
                        if self.form_IV.checkbox_avg.Checked: # averaging of the IV characteristic
                            ...
                            xavg = ((dp_prev.XValue)*(avg_count) + (dp.XValue))/(avg_count+1)
                            yavg = ((dp_prev.YValues[0])*(avg_count) + (dp.YValues[0]))/(avg_count+1)
        ...
        def FormIVClose(self, sender, args):
            ...
            self.form_IV.Hide()            
            if self.running:
                result = Devices_Control.Stop(self.device) #stop AWGs channels if the form is closed
                self.running = False
                ctrl = 1                
                for x in result:
                    ctrl = ctrl*x
