...
serials=['LCRY2345I00080','LCRY2345I00081','LCRY2345I00082','LCRY2345I00083']

masterdev = 1 # from 1 to 4
masterch = 1 # from 1 to 4 for device 1,2,3, for 1 to 3 for the last device (15 channel are used, the last channel of last device is not used and cannot be set as master)
...
def Load(devicelist, freq_signal, n_points, Vmatrix):
    ...
    pars_freq = RecalcSignalFreq(freq_signal, n_points)
    sampl_rate_prescaler = pars_freq[1]
    fsample = Decimal(fsample) #cast from float to Decimal needed for the following methods 
    for dev in devicelist: # I have to set the following parameters otherwise the devices are not sync
        dev.SetSamplingFrequency(fsample, fsample, ClockSource.Internal,fsample)
        dev.PairLeft.SetFrequencyInterpolation(FrequencyInterpolation.Frequency1X)
        dev.PairRight.SetFrequencyInterpolation(FrequencyInterpolation.Frequency1X)
        dev.SetATXSSDeSkewDelay(UInt32(0.))
    ...
    for ch in range(nchannels): #loop over channels
        ...
        if(channelARB == None):
            ...
            if ch==nchannels-1: #last channel
                markers = [1]
...
def GetListOfChannels(devicelist):
    ...
    del(channels[-1]) #it deletes the last channel, the 4th of the 4th device, that it's never used
...
# It initializes the nanovoltmeter Keithley 2182A
def InitVoltmeter():
...
# set impedances, one channel at 50 Ohm, one channel to low impedance and all the others at high impedance
def SetImpedance(device, subsection_status): #subsection_status: list of 1 or 0, 1 means "ON", 0 means "OFF"
    ...
    return(active_ch) #ritorno la posizione del canale settato a 50 Ohm, che sarebbe l'unico che userò per generare tensione
