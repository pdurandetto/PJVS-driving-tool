from math import sin, pi, asin, log
...
def CalcQuantumVolt(Vx, freqRF, R, I_bias, n_subsections): #Ibias in mA
    ...
    Vlsb, Vmax= Eval_Parameters(freqRF)
    ...
    for j in range(len(swap_bits)-1):   #It calculates the voltage of each channel
            ...
            Vtot[j] = (V + R*(swap_bits[j]-swap_bits[j+1])*(I_bias/1000.)) #bias current from mA to A, so divided by 1000
...
def Calc_Sine(amp, freq, phase, offset, n_points, freqRF, R, I_bias, n_subsections):
...
def Calc_Triang(amp, freq, phase, offset, n_points, freqRF, R, I_bias, n_subsections):
...
def Calc_Square(amp, freq, phase, offset, n_points, freqRF, R, I_bias, n_subsections):
...
def Quantum_test(Vlsb, I_bias, n_subsections, R, bit_inversion):

    PJVS_bit_exp=[] #initialize the list that will contain the exponents
    #it gets a list of the exponents of each subarray, INCLUDING the second single junction. This is needed for the binary conversion
    #as an example: [64, 32, 16, 8, 4, 2, 1, 1, 128, 256, 512, 1024, 2048, 4096] ---------> [6, 5, 4, 3, 2, 1, 0, 0, 7, 8, 9, 10, 11, 12]
    for i in range(len(njj_subarray_list)):
            PJVS_bit_exp.append(int(log2(njj_subarray_list[i])))
            
                
    if not bit_inversion:
        qbits = [q for q in quantum_test_bits] # not qbits=quantum_test_bits, because I need to create a copy and not to rename it
    else:
        qbits=[-1*q for q in quantum_test_bits]
